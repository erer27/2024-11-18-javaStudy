 ...
 * 			   => 특별한 경우가 아니면 사용자 정의에서는 많이 사용하지 않는 방법
 * 			   => 시스템 자체에서 처리 => 복구(X), 정상 수행도 불가능
 * 
...
public class 예외처리_예외복구 {
	...
	public static void main(String[] args) throws Exception {
		...
		// 인스턴스는 사용이 불가능 => 사용시에는 반드시 객체생성
		// static은 바로 사용이 가능
		예외처리_예외복구 a=new 예외처리_예외복구();
		// a는 메모리 주소(객체)
		// a => display가 저장됨
//		try {
//			a.display(); // 메소드 호출시에는 예외처리후 사용
//		}catch(Exception ex) {}
		a.display();
